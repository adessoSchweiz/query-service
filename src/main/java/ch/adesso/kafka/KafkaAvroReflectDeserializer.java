...
public class KafkaAvroReflectDeserializer<T> extends AbstractKafkaAvroDeserializer implements Deserializer<T> {
	...
	@SuppressWarnings("unchecked")
	@Override
	protected T deserialize(boolean includeSchemaAndVersion, String topic, Boolean isKey, byte[] payload,
		...
		try {
			...
			DatumReader<T> reader = new ReflectDatumReader<T>(writerSchema, readerSchema);
